...
public class Order {
    ...
    // Idempotency key — a UUID generated by the caller (cart-service or frontend) once per
    // checkout intent. Stored with a UNIQUE constraint so that if the same key arrives twice
    // (network retry, double-click) the second request returns the EXISTING order instead of
    // creating a duplicate.
    //
    // Optional: direct Postman calls without a key still work — they just don't get dedup.
    ...
    // Structured delivery address — embedded as flat columns in the orders table.
    // Stored as a snapshot at order placement time so past orders are never
    // affected by the customer updating their profile address later.
    @Embedded
    ...
    // Timestamp when the "payment expiring soon" warning email was sent.
    // NULL     → warning email not yet sent.
    // NOT NULL → warning was already sent — timeout job skips this order on next run.
    // Prevents the same order from receiving multiple warning emails during the grace window.
